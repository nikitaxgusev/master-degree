...
class TkMplSetup:
    ...
    def main_buttons(self):
        ...
        self.spectro_button = Button(
            bottom_frame, text='Spectro', command=self.spectro)
        self.spectro_button.pack(side='left')

        self.mel_spectro_button = Button(
            bottom_frame, text='Mel-Spectro', command=self.mel_spectro)
        self.mel_spectro_button.pack(side='left')
    ...
    def graph_spectrogram(self, wav_name):
        sound_info, frame_rate = self.get_wav_info(wav_name)
    ...
    def get_wav_info(self, wav_name):
        wav = wave.open(wav_name, 'r')
    ...
    def mel_spectro(self):
        image1 = Image.open('mel-spectrogram.png')
        image1.show()
    ...
    def control_buttons(self):
        ...
        genre_outer_frame = Frame(self.control_frame, bd=1, relief='groove')
        genre_outer_frame.grid(
            row=4, column=0, sticky='ew', pady=PADY_OUTER)
        Label(genre_outer_frame, text='GENRE: HIP-HOP').grid(
            row=4, column=0, stick='w', pady=PADY_INNER_4)
    ...
    def file_options(self):
        ...
        # spectogram building
        self.graph_spectrogram(sound_file.name)

        # mel spectogram building
        import librosa
        import librosa.display
        import IPython.display as ipd
        import matplotlib.pyplot as plt1

        scale_file = sound_file.name
        ...
        plt1.figure(figsize=(25, 10))
        librosa.display.specshow(log_mel_spectrogram, 
                                 x_axis="time",
                                 y_axis="mel", 
                                 sr=sr)
        plt1.colorbar(format="%+2.f")
        plt1.savefig('mel-spectrogram.png')
